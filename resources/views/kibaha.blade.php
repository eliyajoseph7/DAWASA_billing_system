...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Kibaha</li>
  </ol>
</nav> 
...
<body>
 	<div class="container mb-3 mt-3">
        ...
        @if(count($kibaha_people) > 0)
        
            @foreach($kibaha_people -> all() as $kibaha)
            
            <tr>
                <td>{{ ($kibaha -> name) }}</td>
                <td>{{ ($kibaha -> street) }}</td>
                <td>{{ ($kibaha -> metre_no)}}</td>
                <td>{{ ($kibaha -> gender) }}</td>
                <td>{{ ($kibaha -> start) }}</td>
                @if(($kibaha -> status)  == 'notpaid')
