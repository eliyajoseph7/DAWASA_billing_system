...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Bagamoyo</li>
  </ol>
</nav> 
...
<body>
 	<div class="container mb-3 mt-3">
        ...
        @if(count($bagamoyo_people) > 0)
        
            @foreach($bagamoyo_people -> all() as $bagamoyo)
            
            <tr>
                <td>{{ ($bagamoyo -> name) }}</td>
                <td>{{ ($bagamoyo -> street) }}</td>
                <td>{{ ($bagamoyo -> metre_no)}}</td>
                <td>{{ ($bagamoyo -> gender) }}</td>
                <td>{{ ($bagamoyo -> start) }}</td>
                @if(($bagamoyo -> status)  == 'notpaid')
